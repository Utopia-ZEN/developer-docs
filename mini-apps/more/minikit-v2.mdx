...
**New:** Verification works as a mini app and standalone. The IDKit widget will only render outside World App, you can have branching logic based on `MiniKit.isInWorldApp()` if you want to show it in both contexts.
```ts
...
await MiniKit.sendTransaction({
  transactions: [
    {
      data: // If you need to spend funds this would be permit.Approve 
      value: // Hex encoded string
      to: // Hex
    },
    {
      data: // Calldata Hex String, here you would spend the permit approval
      value: // Hex encoded string
      to: // Hex
    },
  ],
